...
CREATE TABLE COMMUNITY(
	...
	updated_at		TIMESTAMP DEFAULT SYSTIMESTAMP,		--DTO는 updateDate
...
CREATE TABLE COMMUNITY_COMMENT(
	...
	status			VARCHAR2(20) DEFAULT 'DISPLAY',
	created_at		TIMESTAMP DEFAULT SYSTIMESTAMP,		--DTO는 commentAt
...
CREATE TABLE INQUIRY (
    ...
    answered_at   TIMESTAMP DEFAULT SYSTIMESTAMP, -- 답변 일시		--DTO는 answerDate
    CONSTRAINT CHK_INQUIRY_STATUS CHECK(status IN('PENDING', 'ANSWERED'))
);
SELECT * FROM INQUIRY;

-- 18. NOTICE_EVENT (공지사항 및 이벤트)
CREATE TABLE NOTICE_EVENT (
    notice_event_id    NUMBER PRIMARY KEY,
    admin_id	VARCHAR2(50) REFERENCES MEMBER(user_id), -- 관리자 user_id
    category	VARCHAR2(50),
    title        VARCHAR2(200) NOT NULL,
    content      CLOB NOT NULL,
...
CREATE SEQUENCE SEQ_NOTICE_EVENT START WITH 1 INCREMENT BY 1;
...
--4) FAQ, NOTICE_EVENT 데이터 수정 시 admin 계정인지 검사하는 프로시저, 트리거
...
-- NOTICE_EVENT용 트리거
CREATE OR REPLACE TRIGGER TRG_NOTICE_EVENT_ADMIN_CHECK
BEFORE INSERT OR UPDATE ON NOTICE_EVENT FOR EACH ROW
BEGIN
    PROC_CHECK_ADMIN_AUTH(:NEW.admin_id);
END;
/
