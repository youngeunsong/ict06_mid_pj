...
public class UserServiceImpl implements UserService {
	...
	public int modifyUserAction(HttpServletRequest request, HttpServletResponse response, Model model)
		...
		// 회원가입 화면 기준과 동일하게 서버에서도 검증
		boolean hasEng = password != null && password.matches(".*[a-zA-Z].*");
		boolean hasNum = password != null && password.matches(".*[0-9].*");
		boolean hasSpe = password != null && password.matches(".*[~!@#$%^&*()_+|<>?:{}].*");
		boolean validLen = password != null && password.length() >= 8 && password.length() <= 20;

		if (!validLen || !hasEng || !hasNum || !hasSpe) {
			return -1;
		}
	...
	public void viewInquiriesAction(HttpServletRequest request, HttpServletResponse response, Model model)
		...
		spring.ict06team1.midpj.SearchCriteria.Paging paging = new spring.ict06team1.midpj.SearchCriteria.Paging(
				pageNum);
	...
	public void cancelReservationAction(HttpServletRequest request, HttpServletResponse response, Model model)
	        throws ServletException, IOException {
	    System.out.println("UserServiceImpl - cancelReservationAction()");
	    ...
	    // 취소 가능한 상태만 허용
	    if (!"PENDING".equals(dto.getStatus()) && !"RESERVED".equals(dto.getStatus())) {
	        model.addAttribute("result", 0);
	        model.addAttribute("msg", "예약대기 또는 예약확정 상태만 취소할 수 있습니다.");
	        return;
	    }

	    int updateCnt = dao.cancelReservation(reservation_id);
