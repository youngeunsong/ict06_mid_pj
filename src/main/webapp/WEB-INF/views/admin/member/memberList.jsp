		...
		<div class="content-wrapper">
			...
			<section class="content">
				<div class="container-fluid">
					...
					<div class="card mb-3">
						<div class="card-body">
							<form action="${path}/memberList.adme" method="get">
									...
									<div class="filter-right">
										<div class="filter-row">
											...
											<div style="display:flex; gap:4px;">
												<span class="tag tag-secondary ${empty sf.status ? 'active' : ''}"
														onclick="setFilter('status', '')">전체</span>
												<span class="tag tag-success ${sf.status eq 'ACTIVE' ? 'active' : ''}"
														onclick="setFilter('status', 'ACTIVE')">정상</span>
												<span class="tag tag-danger ${sf.status eq 'BANNED' ? 'active' : ''}"
														onclick="setFilter('status', 'BANNED')">제재</span>
											</div>
										</div>
										<div class="filter-row">
										...
										<div class="filter-row filter-center">
											<input type="hidden" id="statusInput" name="status" value="${sf.status}">
											<input type="hidden" id="roleInput" name="role" value="${sf.role}">
											<button type="button" onclick="location.href='${path}/memberList.adme'" class="btn btn-dark btn-filter-search">
					...
					<div class="card">
						<div class="card-header">
							...
							<div class="float-right">
								<button type="button" class="tag tag-danger active" onclick="bulkBan()">선택 제재</button>
								<button type="button" class="tag tag-success active" onclick="bulkUnban()">선택 제재 해제</button>
							</div>
						</div>
											...
											<c:forEach var="member" items="${memberList}">
												<tr>
													...
													<td>
														<c:choose>
															<c:when test="${member.status eq 'ACTIVE'}">
																<button type="button" class="tag tag-danger" onclick="banUser('${member.user_id}')">제재</button>
															</c:when>
															<c:otherwise>
						...
						<div class="card-footer clearfix">
							<ul class="pagination pagination-sm m-0 float-right">
								<c:if test="${paging.prev > 0}">
									<li class="page-item">
										<a class="page-link" href="${path}/memberList.adme?pageNum=${paging.prev}&
								...
								<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="i">
									<li class="page-item ${paging.currentPage == i ? 'active' : ''}">
										<a class="page-link" href="${path}/memberList.adme?pageNum=${i}&
											searchType=${sf.searchType}&keyword=${sf.keyword}&status=${sf.status}&role=${sf.role}">${i}</a>
									</li>
								</c:forEach>
								<c:if test="${paging.next > 0}">
									<li class="page-item">
										<a class="page-link" href="${path}/memberList.adme?pageNum=${paging.next}&
											searchType=${sf.searchType}&keyword=${sf.keyword}&status=${sf.status}&role=${sf.role}">»</a>
									</li>
								</c:if>
							</ul>
