...
<div class="row"> 
    <c:forEach var="fest" items="${festivalList}">
        <c:set var="placeId" value="${fest.placeDTO.place_id}" />
        ...
        <div class="${finalCardWrapClass}">
            <div class="place-card-wrap position-relative shadow-sm border rounded bg-white h-100">
                ...
                <a href="javascript:void(0);" 
                   ...
                   ondblclick="location.href='${path}/festivalDetail.fe?place_id=${placeId}';">
                    ...
                    <div class="place-card__body p-3">
                        <div class="place-card__title fw-bold fs-5 mb-2 text-truncate">${fest.placeDTO.name}</div>
                        <div class="place-card__address text-muted small mb-3 text-truncate">
                            <i class="bi bi-geo-alt-fill text-danger"></i> ${fest.placeDTO.address}
                        </div>
                        <div class="place-card__meta d-flex justify-content-between align-items-center border-top pt-2">
                            <div class="d-flex gap-3">
                                <span><i class="fa-regular fa-eye text-primary"></i> ${fest.placeDTO.view_count}</span>
                                <span><i class="fa-regular fa-star text-warning"></i> <c:out value="${fest.placeDTO.avg_rating}" default="0.0"/></span>
                                <span><i class="fa-regular fa-comment"></i> <c:out value="${fest.placeDTO.review_count}" default="0"/></span>
                            </div>
                        </div>
                    </div>
                </a>
