...
<!-- 페이지 전용 CSS -->
<link rel="stylesheet"
	href="${path}/resources/css/user/surveyReview.css">
</head>
		...
		<section class="survey-review-section">
			...
			<form id="surveyForm" action="${path}/surveyReviewAction.rv"
				...
				<input type="hidden" name="reservation_id"
					value="${hiddenReservationId}"> <input type="hidden"
				...
				<div class="form-card">
					...
					<div class="form-group">
						<label class="form-label">1. 맛침내 서비스 이용 전반에 대한 만족도를
							선택해주세요. (10점 만점)</label>
					...
					<!-- 2. 불편사항 -->
					<div class="form-group">
						<label for="inconvenience" class="form-label">2. 맛침내 이용 중
							불편하거나 아쉬웠던 점이 있었다면 자유롭게 작성해주세요.</label>
						<textarea id="inconvenience" name="inconvenience"
							class="form-control-custom" placeholder="예) 환불 요청이 안돼요 "></textarea>
					</div>

					<!-- 3. 정보 신뢰도 -->
					<div class="form-group">
						<label class="form-label">3. 맛침내에서 제공된 정보의 신뢰도는 어땠나요? (10점
							만점)</label>
						...
						<div class="form-hint">장소 정보, 이용 안내, 예약 정보 등을 기준으로 생각해주세요.</div>
					</div>

					<!-- 4. 개선사항 -->
					<div class="form-group">
						<label for="improvements" class="form-label">4. 맛침내의
							개선되었으면 하는 점이 있다면 적어주세요.</label>
					...
					<div class="form-group survey-last-group">
						<label class="form-label">5. 맛침내를 다른 사람에게 추천할 의향이 있나요?
							(10점 만점)</label>
				...
				<div class="form-card">
					...
					<!-- 별점 -->
					<div class="form-group">
						<label class="form-label">이용하신 장소에 대한 별점을 선택해주세요. (5점 만점)</label>
						<div class="score-wrap" data-target="page_rating">
							<c:forEach begin="1" end="5" var="i">
								<button type="button" class="star-item" data-value="${i}">★${i}</button>
							</c:forEach>
						</div>
						<div class="star-guide">별점은 해당 장소의 평균 점수에 반영됩니다.</div>
					</div>
				...
				<div class="button-area">
					...
					<button type="submit" class="btn-point">제출하기</button>
				</div>
			</form>
		</section>
							...
							if (form) {
								form
										.addEventListener(
												"submit",
												function(e) {
													const satisfaction = document
															.getElementById("page_satisfaction_score").value;
													const reliability = document
															.getElementById("page_info_reliability_score").value;
													const nps = document
															.getElementById("page_nps_score").value;
													const rating = document
															.getElementById("page_rating").value;
