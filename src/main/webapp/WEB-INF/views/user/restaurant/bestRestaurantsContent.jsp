      ...
      <div class="rk-sectionSub">
        <c:choose>
          <c:when test="${not empty currentRegion and currentRegion ne 'all'}">
            ${currentRegion} 지역 기준 인기 맛집 <%-- 특정 지역 선택 시 해당 지역 안내 문구 출력 --%>
          </c:when>
          <c:otherwise>
            전체 지역 기준 인기 맛집 <%-- 지역 미선택 또는 전체 선택 시 기본 문구 출력 --%>
          </c:otherwise>
        </c:choose>
      </div>
    </c:when>
  ...
  <div class="rk-topWrap">
    ...
    <c:forEach var="item" items="${topList}" begin="0" end="0">
      <div class="rk-top1 mb-4">
        ...
        <jsp:include page="/WEB-INF/views/common/card/BigRestCard.jsp"/> <%-- 공통 카드 컴포넌트 재사용 --%>
      </div>
    </c:forEach>
