...
// [랭킹 페이지 초기 설정]
// 숙소 랭킹 페이지 진입 시 탭, 필터, 더보기 기능을 한 번에 연결하기 위해 사용
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateFilterVisibility() {
        if (regionSelect) {
            regionSelect.style.display =
                (state.tab === "region") ? "block" : "none"; // 지역 탭에서만 지역 선택 표시
        }
    }

    // [더보기 카드 HTML 생성]
    // 더보기 버튼으로 불러온 데이터를 기존 BigAccCard 스타일과 동일하게 출력하기 위해 사용
    ...
    // [탭별 콘텐츠 AJAX 로딩]
    // 실시간/지역 탭 전환 시 페이지 전체 이동 없이 랭킹 영역만 교체하기 위해 사용
    function loadTabContent(tabType) {
        const url =
            path +
            "/bestAccommodationsTabAjax.ac?tab=" + encodeURIComponent(tabType) +
